            ...
            if ( isset($xml_data['Valute']) ) {
                foreach ($xml_data['Valute'] as $Valute) {
                    ...
                    if ( isset($currencies[$Valute['CharCode']]) && isset($Valute['Value'])) {
                        ...
                        $value = $value / $Valute['Nominal'];
                        $value = 1 / $value;
                         
                        if ($value) {
                            $this->db->query("UPDATE " . DB_PREFIX . "currency SET value = '" . (float)$value . "', date_modified = '" .  $this->db->escape($date_modified) . "' WHERE code = '" . $this->db->escape($Valute['CharCode']) . "'");
                        }

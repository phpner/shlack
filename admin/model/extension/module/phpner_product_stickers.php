...
class ModelExtensionModulePhpnerProductStickers extends Model {
    public function makeDB() {
        $sql1 = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "phpner_product_stickers` (";
        ...
        $sql2 = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "phpner_product_stickers_description` (";
        ...
        $this->db->query("ALTER TABLE `" . DB_PREFIX . "product` ADD `phpner_product_stickers` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL ;");
    }
    
    public function removeDB() {
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "phpner_product_stickers`;");
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "phpner_product_stickers_description`;");
        $this->db->query("ALTER TABLE `" . DB_PREFIX . "product` DROP `phpner_product_stickers` ;");
    }
    ...
    public function updateStickers($module_categories, $module_stickers, $filters) {
        ...
        if ($results) {
            foreach ($results as $product_id) {
                $this->db->query("UPDATE " . DB_PREFIX . "product SET phpner_product_stickers = '" . $this->db->escape(serialize($module_stickers)) . "' WHERE product_id = '" . (int) $product_id . "'");
            }
            return true;
        } else {
            return false;

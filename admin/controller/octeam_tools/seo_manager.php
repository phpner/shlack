<?php
class Controllerphpner_eamToolsSeoManager extends Controller {
    private $error = array();

    public function index() {
        $this->load->language('phpner_eam_tools/seo_manager');
        $this->document->setTitle($this->language->get('heading_title'));
        $this->load->model('phpner_eam_tools/seo_manager');
        $this->getForm();
    }
    ...
    protected function getForm() {
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('text_phpner_eam_toolset'),
            'href' => $this->url->link('phpner_eam/toolset', 'token=' . $this->session->data['token'], 'SSL')
        );

        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('phpner_eam_tools/seo_manager', 'token=' . $this->session->data['token'] . $url, 'SSL')
        );
       
        $data['delete'] = $this->url->link('phpner_eam_tools/seo_manager/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
        
        $data['save'] = $this->url->link('phpner_eam_tools/seo_manager/update', 'token=' . $this->session->data['token'] . $url, 'SSL');
        
        $data['clear'] = $this->url->link('phpner_eam_tools/seo_manager/clear', 'token=' . $this->session->data['token'] . $url, 'SSL');
        
        $data['cancel'] = $this->url->link('phpner_eam/toolset', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $url_alias_total = $this->model_phpner_eam_tools_seo_manager->getTotalUrlAalias($filter_data);

        $results = $this->model_phpner_eam_tools_seo_manager->getUrlAaliases($filter_data);
        ...
        $data['sort_query'] = $this->url->link('phpner_eam_tools/seo_manager', 'token=' . $this->session->data['token'] . '&sort=query' . $url, 'SSL');
        $data['sort_keyword'] = $this->url->link('phpner_eam_tools/seo_manager', 'token=' . $this->session->data['token'] . '&sort=keyword' . $url, 'SSL');
        ...
        $pagination->url = $this->url->link('phpner_eam_tools/seo_manager', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('phpner_eam_tools/seo_manager.tpl', $data));
    }
        ...
        public function update() {
                ...
                if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
                        $this->model_phpner_eam_tools_seo_manager->updateUrlAlias($this->request->post);
                        $this->session->data['success'] = $this->language->get('text_success');
                    
                    $this->response->redirect($this->url->link('phpner_eam_tools/seo_manager', 'token=' . $this->session->data['token'] . $url, 'SSL'));
                }
    ...
    public function delete() {
                ...
                if (isset($this->request->post['selected']) && $this->validate()) {
                        foreach ($this->request->post['selected'] as $url_alias_id) {
                                $this->model_phpner_eam_tools_seo_manager->deleteUrlAlias($url_alias_id);
                        }
                        $this->session->data['success'] = $this->language->get('text_success');
                }
    ...
    protected function validatePermission() {
        return $this->user->hasPermission('modify', 'phpner_eam_tools/seo_manager');
    }
}
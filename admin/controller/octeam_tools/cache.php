<?php 
class Controllerphpner_eamToolsCache extends Controller {
    private $error = array();
    private $version = '1.2.ocs2101';

    public function __construct($registry) {
        parent::__construct($registry);
        $this->load->language('phpner_eam_tools/cache');
    }
    ...
    public function index() {
        ...
        $data['action'] = str_replace('&amp;', '&', $this->url->link('phpner_eam_tools/cache/remove', 'token=' . $this->session->data['token'], 'SSL'));
        $data['back'] = $this->url->link('phpner_eam/toolset', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'  => $this->url->link('phpner_eam_tools/cache', 'token=' . $this->session->data['token'], 'SSL')
          );
        ...
        $this->response->setOutput($this->load->view('phpner_eam_tools/cache.tpl', $data));
    }

    public function remove() {
      $json = array();

      if ($this->user->hasPermission('modify', 'phpner_eam_tools/cache')) {

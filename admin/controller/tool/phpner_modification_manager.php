...
class ControllerToolphpner_ModificationManager extends Controller {
	public function index() {
    $data = array();

    $this->load->model('tool/phpner_modification_manager');

	  $data = array_merge($data, $this->load->language('tool/phpner_modification_manager'));

	  $this->document->setTitle($this->language->get('heading_title'));

	  $this->document->addScript('view/javascript/phpner/codemirror/lib/codemirror.js');
	  $this->document->addScript('view/javascript/phpner/codemirror/lib/css.js');
    $this->document->addScript('view/javascript/phpner/codemirror/mode/xml/xml.js');
    $this->document->addStyle('view/javascript/phpner/codemirror/lib/codemirror.css');
    $this->document->addStyle('view/javascript/phpner/codemirror/theme/base16-dark.css');
    ...
    $filter_data = array(
      'filter_author_group' => true
		...
		$modification_authors = $this->model_tool_phpner_modification_manager->getModifications($filter_data);
    ...
    $data['authors_archive'] = $this->get_authors('archive');
		...
		$this->response->setOutput($this->load->view('tool/phpner_modification_manager', $data));
	}
    ...
    $phpInfo = preg_replace('/<?([\w]|<|\/|>)*(a:|!Dphpner_YPE|<html|title|body|img)(.+)?\\n?/', '', $phpInfo);
    ...
    $filter_data = array(
      'filter_author' => 'Phpner',
      'filter_author_group' => false
    );

    $modifications = $this->model_tool_phpner_modification_manager->getModifications($filter_data);
	...
	public function get_authors($mod_type) {
      ...
      $files = glob(DIR_SYSTEM . 'storage/phpner_modification_archive/*.ocmod.xml*');
    }
	...
	public function history_system() {
      ...
      $data = array_merge($data, $this->language->load('tool/phpner_modification_manager'));
      ...
      if ($xmls) {
        ...
        foreach ($xmls as $xml) {
          ...
          if (($this->request->get['filter_author'] == $author || $this->request->get['filter_author'] == '-all-') && (!empty($this->request->get['filter_name']) && (stristr($name, $this->request->get['filter_name'])) || $this->request->get['filter_name'] == '')) {
            ...
            $check_modification = $this->model_tool_phpner_modification_manager->getTotalModificationByCode($code);
      ...
      $pagination->url = $this->url->link('tool/phpner_modification_manager/history_system', 'token=' . $this->session->data['token'].'&filter_author='.$this->request->get['filter_author'].'&filter_name='.$this->request->get['filter_name'].'&page={page}', true);
      ...
      $this->response->setOutput($this->load->view('tool/phpner_modification_manager_history_system', $data));
    }
  }
  ...
  public function history_installed() {
      ...
      $modification_total = $this->model_tool_phpner_modification_manager->getTotalModifications();

      $results = $this->model_tool_phpner_modification_manager->getModifications($filter_data);
      ...
      $pagination->page = $page;
      $pagination->limit = $admin_limit;
      $pagination->url = $this->url->link('tool/phpner_modification_manager/history_installed', 'token=' . $this->session->data['token'].'&filter_author='.$this->request->get['filter_author'].'&filter_name='.$this->request->get['filter_name'].'&page={page}', true);
      ...
      $this->response->setOutput($this->load->view('tool/phpner_modification_manager_history_installed', $data));
    }
  }
  ...
  public function history_archive() {
      ...
      if (!is_dir(DIR_SYSTEM . 'storage/phpner_modification_archive')) {
        mkdir(DIR_SYSTEM . 'storage/phpner_modification_archive', 0755, true);
      }
      ...
      $pagination->page = $page;
      $pagination->limit = $admin_limit;
      $pagination->url = $this->url->link('tool/phpner_modification_manager/history_archive', 'token=' . $this->session->data['token'].'&filter_author='.$this->request->get['filter_author'].'&filter_name='.$this->request->get['filter_name'].'&page={page}', true);

      $data['pagination'] = $pagination->render();

      $data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1)*$admin_limit) + 1 : 0, ((($page - 1)*$admin_limit) > ($history_total - $admin_limit)) ? $history_total : ((($page - 1)*$admin_limit) + $admin_limit), $history_total, ceil($history_total/$admin_limit));

      $this->response->setOutput($this->load->view('tool/phpner_modification_manager_history_archive', $data));
    }
  }
  ...
  public function change_status() {
    ...
    $this->language->load('tool/phpner_modification_manager');

    $this->load->model('extension/modification');

    if (!$this->user->hasPermission('modify', 'tool/phpner_modification_manager')) {
    ...
    if (!$this->user->hasPermission('modify', 'tool/phpner_modification_manager')) {
      ...
      if (isset($this->request->request['modification']) && isset($this->request->request['mod_type'])) {
        if ($this->request->request['mod_type'] == 'system') {
          ...
          $file = DIR_SYSTEM.'storage/phpner_modification_archive/'.$this->request->request['modification'];
    ...
    if (!$this->user->hasPermission('modify', 'tool/phpner_modification_manager')) {
      ...
      if (isset($this->request->request['selected']) && isset($this->request->request['mod_type'])) {
        if ($this->request->request['mod_type'] == 'system') {
          ...
          foreach ($this->request->request['selected'] as $modification) {
            $file = DIR_SYSTEM.'storage/phpner_modification_archive/'.$modification;
    ...
    if (!$this->user->hasPermission('modify', 'tool/phpner_modification_manager')) {
      ...
      if (!empty($modification) && !empty($mod_type)) {
        if ($mod_type == 'system') {
          ...
          $copy_to = DIR_SYSTEM . 'storage/phpner_modification_archive/';
          ...
          $modification_info = $this->model_tool_phpner_modification_manager->getModification($modification);
        ...
        if ($this->request->request['mod_type'] == 'system') {
          ...
          $modification_info = $this->model_tool_phpner_modification_manager->getModification($this->request->request['modification']);
    ...
    if (!$this->user->hasPermission('modify', 'tool/phpner_modification_manager')) {
      ...
      if (isset($this->request->request['modification']) && isset($this->request->request['mod_type'])) {
        if ($this->request->request['mod_type'] == 'system') {
          ...
          $copy_from = DIR_SYSTEM . 'storage/phpner_modification_archive/';
          $copy_to = DIR_SYSTEM;
          ...
          if (!isset($json['error'])) {
            ...
            //if (is_file(DIR_SYSTEM. 'storage/phpner_modification_archive/'.$file) && file_exists(DIR_SYSTEM. 'storage/phpner_modification_archive/'.$file)) {
            //  unlink(DIR_SYSTEM. 'storage/phpner_modification_archive/'.$file);
            //}
          ...
          $file_code = file_get_contents(DIR_SYSTEM . 'storage/phpner_modification_archive/'.$this->request->request['modification']);
          $dom = new DOMDocument('1.0', 'UTF-8');
          $dom->loadXml(html_entity_decode($file_code));

          if ($this->request->request['mod_type'] == 'installed' && $this->model_tool_phpner_modification_manager->getTotalModificationByCode(($dom->getElementsByTagName('code')->length > 0) ? $dom->getElementsByTagName('code')->item(0)->nodeValue : '') > 0) {
          ...
          if (!isset($json['error'])) {
            ...
            $this->model_tool_phpner_modification_manager->addModification($filter_data);

            # можно удалять архивный файл после архивирования, для этого раскомментируйте код ниже
            //if (is_file(DIR_SYSTEM. 'storage/phpner_modification_archive/'.DIR_SYSTEM . 'storage/phpner_modification_archive/'.$this->request->request['modification']) && file_exists(DIR_SYSTEM. 'storage/phpner_modification_archive/'.DIR_SYSTEM . 'storage/phpner_modification_archive/'.$this->request->request['modification'])) {
            //  unlink(DIR_SYSTEM. 'storage/phpner_modification_archive/'.DIR_SYSTEM . 'storage/phpner_modification_archive/'.$this->request->request['modification']);
            //}
  ...
  public function modification_info() {
      ...
      if ($this->request->get['mod_type'] == 'system') {
        ...
        $data['xml'] = (is_file( DIR_SYSTEM . 'storage/phpner_modification_archive/'.$this->request->get['modification'])) ? htmlentities(file_get_contents( DIR_SYSTEM . 'storage/phpner_modification_archive/'.$this->request->get['modification'])) : $this->language->get('error_failed_load_file');
      } elseif ($this->request->get['mod_type'] == 'installed') {
        $modification_info = $this->model_tool_phpner_modification_manager->getModification($this->request->get['modification']);
    ...
    $this->response->setOutput($this->load->view('tool/phpner_modification_manager_info', $data));
  }
  ...
  public function edite_selected() {
    ...
    if (!$this->user->hasPermission('modify', 'tool/phpner_modification_manager')) {
      ...
      if (isset($this->request->post['code']) && isset($this->request->post['modification']) && isset($this->request->post['mod_type'])) {
        ...
        if ($this->request->post['mod_type'] == 'archive' && file_exists(DIR_SYSTEM.'storage/phpner_modification_archive/'.$this->request->post['modification'].'.ocmod.xml')) {
        ...
        if (!isset($json['error'])) {
          if ($this->request->post['mod_type'] == 'system') {
            ...
            if (is_file(DIR_SYSTEM.'storage/phpner_modification_archive/'.$this->request->post['modification'])) {

              file_put_contents(DIR_SYSTEM.'storage/phpner_modification_archive/'.$this->request->post['modification'], html_entity_decode($this->request->post['code']));
            ...
            $modification_info = $this->model_tool_phpner_modification_manager->getModification($this->request->post['modification']);
            ...
            if ($modification_info) {
              ...
              $this->model_tool_phpner_modification_manager->editModification($this->request->post['modification'], $filter_data);
  ...
  public function add_modification_index() {
    ...
    $this->response->setOutput($this->load->view('tool/phpner_modification_manager_add', $data));
  }
  ...
  public function add_modification_action() {
    ...
    if (!$this->user->hasPermission('modify', 'tool/phpner_modification_manager')) {
      ...
      if (isset($this->request->request['code']) && isset($this->request->request['modification']) && isset($this->request->request['mod_type'])) {
        ...
        if ($this->request->request['mod_type'] == 'archive' && file_exists(DIR_SYSTEM.'storage/phpner_modification_archive/'.$this->request->request['modification'].'.ocmod.xml')) {
          $json['error'] = $this->language->get('error_modification_exist_system');
        }
        ...
        if (!isset($json['error'])) {
          if ($this->request->request['mod_type'] == 'system') {
            ...
            file_put_contents(DIR_SYSTEM.'storage/phpner_modification_archive/'.$this->request->request['modification'].'.ocmod.xml', html_entity_decode($this->request->request['code']));

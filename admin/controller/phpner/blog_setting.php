...
class ControllerPhpnerBlogSetting extends Controller {
    private $error = array();
    
    public function index() {
        $data = array();
        
        $data = array_merge($data, $this->load->language('phpner/blog_setting'));
        
        $this->load->model('setting/setting');
        $this->load->model('extension/extension');
        $this->load->model('user/user_group');
        $this->load->model('phpner/blog_setting');
        ...
        if (!in_array('phpner_blog_setting', $this->model_extension_extension->getInstalled('phpner'))) {
            
            $this->model_phpner_blog_setting->installTables();
            
            $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'phpner/blog_setting');
            $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'phpner/blog_setting');
            $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'phpner/blog_category');
            $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'phpner/blog_category');
            $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'phpner/blog_article');
            $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'phpner/blog_article');
            $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'phpner/blog_comments');
            $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'phpner/blog_comments');
            ...
            $this->model_setting_setting->editSetting('phpner_blog_setting', array(
                'phpner_blog_setting_data' => array(
                    'status' => '1',
            ...
            $this->model_extension_extension->install('phpner', 'phpner_blog_setting');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('phpner_blog_setting', $this->request->post);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('phpner/blog_setting', 'token=' . $this->session->data['token'], 'SSL')
        );
        
        $data['action']    = $this->url->link('phpner/blog_setting', 'token=' . $this->session->data['token'], 'SSL');
        $data['uninstall'] = $this->url->link('phpner/blog_setting/uninstall', 'token=' . $this->session->data['token'], 'SSL');
        $data['cancel']    = $this->url->link('common/dashboard', 'token=' . $this->session->data['token'], 'SSL');
        
        if (isset($this->request->post['phpner_blog_setting_data'])) {
            $data['phpner_blog_setting_data'] = $this->request->post['phpner_blog_setting_data'];
        } else {
            $data['phpner_blog_setting_data'] = $this->config->get('phpner_blog_setting_data');
        }
        ...
        $this->response->setOutput($this->load->view('phpner/blog_setting.tpl', $data));
    }
    
    public function uninstall() {
        $this->load->language('phpner/blog_setting');
        
        $this->load->model('extension/extension');
        
        $this->model_extension_extension->uninstall('phpner', 'phpner_blog_setting');
        
        $this->load->model('phpner/blog_setting');
        
        $this->model_phpner_blog_setting->deleteTables();
    ...
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'phpner/blog_setting')) {
            $this->error['warning']         = $this->language->get('error_permission');
            $this->session->data['warning'] = $this->language->get('error_permission');
        }
        
        if (isset($this->request->post['phpner_blog_setting_data']['language'])) {
            foreach ($this->request->post['phpner_blog_setting_data']['language'] as $language_id => $value) {
        ...
        foreach ($this->request->post['phpner_blog_setting_data'] as $main_key => $field) {

...
class ControllerExtensionModulePhpnerProductViewed extends Controller {
    public function index($setting) {
        ...
        if (isset($this->session->data['phpner_product_viewed']) && !empty($this->session->data['phpner_product_viewed']) && count($this->session->data['phpner_product_viewed']) > 2) {
            
            $products = array_slice(array_unique($this->session->data['phpner_product_viewed']), 0, $setting['limit']);
        ...
        if ($data['products']) {
            return $this->load->view('extension/module/phpner_product_viewed', $data);
        }
    }
}
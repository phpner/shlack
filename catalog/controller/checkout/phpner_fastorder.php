...
class ControllerCheckoutPhpnerFastorder extends Controller {
    public function index() {
        $data = array();
        
        $phpner_fastorder_data                         = $this->config->get('phpner_fastorder_data');
        $data['phpner_fastorder_data']                 = $phpner_fastorder_data;
        $data['phpner_advanced_options_settings_data'] = $this->config->get('phpner_advanced_options_settings_data');
        
        if (!isset($phpner_fastorder_data['status'])) {
            $this->response->redirect($this->url->link('checkout/checkout'));
        } elseif (isset($phpner_fastorder_data['status']) && $phpner_fastorder_data['status'] == 0) {
            $this->response->redirect($this->url->link('checkout/checkout'));
        }
        
        $this->load->language('checkout/phpner_fastorder');
        ...
        if (!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) {
            $this->response->redirect($this->url->link('checkout/phpner_fastorder/empty_cart'));
        }
        ...
        if ($this->config->get('config_cart_weight') && (isset($phpner_fastorder_data['cart_weight']) && $phpner_fastorder_data['cart_weight'] == 1)) {
        ...
        $data['mask'] = ($phpner_fastorder_data['mask']) ? $phpner_fastorder_data['mask'] : '';
        ...
        $this->response->setOutput($this->load->view('checkout/phpner_fastorder/fastorder', $data));
    }
    ...
    public function shipping_address($render = true, &$data = array()) {
        ...
        if (isset($this->session->data['shipping_country_id'])) {
            ...
            $data['country_id'] = $phpner_fastorder_data['default_country'];
        }
    ...
    public function shipping_method($render = true, &$data = array()) {
        ...
        if ($render !== false) {
            $this->response->setOutput($this->load->view('checkout/phpner_fastorder/shipping_method', $data));
        }
    }
    ...
    public function payment_method($render = true, &$data = array()) {
        ...
        if ($render !== false) {
            $this->response->setOutput($this->load->view('checkout/phpner_fastorder/payment_method', $data));
        }
    }
    ...
    public function checkout($render = true, &$data = array()) {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('checkout/phpner_fastorder', '', 'SSL'),
            'separator' => $this->language->get('text_separator')
        );
    ...
    public function login_validate($data = array()) {
        ...
        if ($this->customer->isLogged()) {
            $json['redirect'] = $this->url->link('checkout/phpner_fastorder', '', 'SSL');
        }
    ...
    public function guest_validate() {
        ...
        if (!$json) {
            ...
            if ($phpner_fastorder_data['lastname'] == 1) {
            ...
            if (isset($this->request->post['email'])) {
                if ((isset($phpner_fastorder_data['email']) && $phpner_fastorder_data['email'] == 2)) {
                ...
                if ((isset($phpner_fastorder_data['email']) && $phpner_fastorder_data['email'] == 1)) {
                    if ((utf8_strlen(trim($this->request->post['email'])) > 1)) {
                        if (!preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $this->request->post['email'])) {
                            $json['error']['email'] = $this->language->get('error_valid_email');
                        }
            ...
            if ($phpner_fastorder_data['address_1'] == 2) {
            ...
            if ($phpner_fastorder_data['address_2'] == 2) {
            ...
            if ($phpner_fastorder_data['city'] == 2) {
    ...
    public function cart($render = true, &$data = array()) {
        ...
        foreach ($totals as $total) {
            if ($total['code'] == 'total' && (isset($phpner_fastorder_data['minimum_order']) && !empty($phpner_fastorder_data['minimum_order']) && $phpner_fastorder_data['minimum_order'] > $total['value'])) {
                $data['error_warning'] = sprintf($this->language->get('error_minimum_sum'), $this->currency->format($phpner_fastorder_data['minimum_order'], $this->session->data['currency']));
            }
            
            if ($total['code'] == 'total' && (isset($phpner_fastorder_data['max_order']) && !empty($phpner_fastorder_data['max_order']) && $phpner_fastorder_data['max_order'] < $total['value'])) {
                $data['error_warning'] = sprintf($this->language->get('error_maximum_sum'), $this->currency->format($phpner_fastorder_data['max_order'], $this->session->data['currency']));
            }
        ...
        $data['checkout'] = $this->url->link('checkout/phpner_fastorder', '', 'SSL');
        
        $data['checkout_buttons'] = array();
        
        $this->response->setOutput($this->load->view('checkout/phpner_fastorder/cart', $data));
    }
    ...
    public function status_cart() {
        ...
        if (!$this->cart->hasProducts()) {
            $json['redirect'] = $this->url->link('checkout/phpner_fastorder');
        }
    ...
    public function confirm($render = true, &$data = array()) {
        ...
        if ($this->cart->hasShipping()) {
            ...
            if (!isset($this->session->data['shipping_address'])) {
                $redirect = $this->url->link('checkout/phpner_fastorder', '', 'SSL');
            }
        ...
        if (isset($this->request->post) && !empty($this->request->post)) {
            ...
            foreach ($this->cart->getProducts() as $product) {
                ...
                foreach ($product['option'] as $option) {
                    ...
                    $phpner_product_option_value_id_val = explode("|", $option['product_option_value_id']);
                    
                    if ($option['type'] == 'phpner_quantity') {
                        $phpner_product_option_value_id = $phpner_product_option_value_id_val[0];
                    } else {
                        $phpner_product_option_value_id = $option['product_option_value_id'];
                    }
                    
                    $option_data[] = array(
                        'product_option_id' => $option['product_option_id'],
                        'product_option_value_id' => $phpner_product_option_value_id,
                        'option_id' => $option['option_id'],
                        'option_value_id' => $option['option_value_id'],
                        'name' => $option['name'],
                        'sku' => (isset($option['sku'])) ? $option['sku'] : '',
                        'model' => (isset($option['model'])) ? $option['model'] : '',
                        'phpner_quantity_value' => (isset($phpner_product_option_value_id_val[1])) ? $phpner_product_option_value_id_val[1] : '',
                        'value' => $option['value'],
                        'type' => $option['type']
                    );
                }
    ...
    public function empty_cart() {
        if (!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) {
            ...
            $this->response->redirect($this->url->link('checkout/phpner_fastorder'));
        }
    }
}
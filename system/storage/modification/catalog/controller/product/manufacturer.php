...
class ControllerProductManufacturer extends Controller {
	public function index() {
        ...
        if (isset($phpner_data['showmanlogos']) && $phpner_data['showmanlogos'] == "on") {
          $data['show_logo'] = 1;
        } else {
          $data['show_logo'] = 0;
        }
	...
	public function info() {
      ...
      // SEO phpner_ product filter: end
        ...
        // phpner_advanced_attributes_settings end
			...
			$pagination->url = ((isset($seo_data_info) && $seo_data_info) || $phpner_product_filter_status) ? (rtrim($phpner_server, "/"). '&page={page}') : ($this->url->link('product/manufacturer/info', 'manufacturer_id=' . $this->request->get['manufacturer_id'] .  $url . '&page={page}'));
			...
			if ($page == 1) {
			    (isset($seo_data_info) && $seo_data_info && isset($phpner_server)) ? ($this->document->addLink($phpner_server, 'canonical')) : ($this->document->addLink($this->url->link('product/manufacturer/info', 'manufacturer_id=' . $this->request->get['manufacturer_id'], true), 'canonical'));
			} elseif ($page == 2) {
			    (isset($seo_data_info) && $seo_data_info && isset($phpner_server)) ? '' : ($this->document->addLink($this->url->link('product/manufacturer/info', 'manufacturer_id=' . $this->request->get['manufacturer_id'], true), 'prev'));
			} else {
			    (isset($seo_data_info) && $seo_data_info && isset($phpner_server)) ? '' : ($this->document->addLink($this->url->link('product/manufacturer/info', 'manufacturer_id=' . $this->request->get['manufacturer_id'] . $url . '&page='. ($page - 1), true), 'prev'));
			}

			if ($limit && ceil($product_total / $limit) > $page) {
			    (isset($seo_data_info) && $seo_data_info && isset($phpner_server)) ? '' : ($this->document->addLink($this->url->link('product/manufacturer/info', 'manufacturer_id=' . $this->request->get['manufacturer_id'] . $url . '&page='. ($page + 1), true), 'next'));
			}
